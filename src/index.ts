...
function getIgnoredEpisodesFromRequest(req: Request): string[] {
  ...
  if (req.cookies && req.cookies.ignoredEpisodes) {
    ...
    for (const r of rawIgnoredEpisodes) ignoredEpisodes.push(r);
  }
...
function getIgnoreEpisodeFromQuery(req: Request): [string, string] | undefined {
  if (req.query.ignore) {
    ...
    if (typeof i === "string" && typeof n === "string") {
      ...
      logger.warn(
        `Unknown query parameter value for 'ignored': ${req.query.ignore}`
      );
    }
  }
  return undefined;
...
async function main() {
  ...
  if (
    ...
    spotify = await Spotify.createFromCredentials(
      spotifyClientId,
      spotifyClientSecret,
      logger
    );
    ...
    throw new Error(
      "Spotify client id or secret is not set. Please set SPOTIFY_SHUFFLE_CLIENT_ID and SPOTIFY_SHUFFLE_CLIENT_SECRET."
    );
  }
  ...
  app.get("/", async (req, res) => {
    ...
    const nonIgnoredAlbums = albums.filter(
      (a) => !ignoredEpisodes.some((i: string) => a.id === i)
    );
  ...
  if (spotifyArtistId === undefined || spotifyArtistId === null) {
    throw new Error(
      "The artist id has not bben set. Please set SPOTIFY_SHUFFLE_ARTIST_ID."
    );
  }
